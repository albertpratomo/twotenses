...
<template>
  <section
    class="
      relative container-fluid flex-center flex-col h-screen bg-black
      px-container text-center text-gray
    "
  >
    <div
      class="absolute top-0 right-0 size-4"
      @click="next()"
    />

    <img
      v-if="state.image"
      class="mb-8 skeleton rounded-sm"
    ...
    <textarea
      ...
      v-model="answer"
      class="
        mt-8 w-80 rounded-sm border bg-transparent p-3
    ...
    <div class="flex gap-4">
      <button
        ...
        :key="i"
        class="
          mt-8 rounded-sm border px-4 py-2
